...
3. **Frontmatter ausfüllen** (der Block zwischen den `---`-Linien oben):
   ...
   - `cover` (optional, nur bei `format: video` relevant): Dateiname eines
     Bilds im selben Asset-Ordner (z. B. `cover.jpg`), das als Thumbnail für
     das Reel verwendet wird, statt Frame 0 des Videos. Grund: unsere Reels
     starten oft mit einem kurzen Schwarzbild-Intro, das sonst als hässliches
   ...
   - `publish_at`: Datum + Uhrzeit im Format `JJJJ-MM-TTTHH:MM:SS+02:00`
     ...
     der Zukunft liegen** – ein Zeitpunkt, der schon vorbei ist, wird von
     Metricool abgelehnt und der Post landet auf `status: error`.
   - `anlass` und `stufe`: die zwei Kürzel aus dem Briefing zu dem Post. Wenn
     du keins hast, frag Ben – nicht raten und nicht weglassen. Sie sind der
     Grund, warum wir später überhaupt sagen können, welche Inhalte etwas
     gebracht haben.
...
5. **Wenn der Post wirklich fertig zum Posten ist**, ändere ganz oben im
   ...
   **Achtung Uhrzeit:** Der Lauf schaut nur einmal täglich um 07:00 Uhr nach.
   Setzt du `ready` erst danach, greift es erst am nächsten Morgen. Plane
   also Vorlauf ein.
...
- `error` – etwas ist beim Einplanen schiefgelaufen (meistens: die
  `publish_at`-Zeit lag schon in der Vergangenheit). Sag in dem Fall kurz
  Bescheid, dann schaut sich das jemand an.
...
- Dieses Repo ist **öffentlich auf GitHub** (aus rein technischen Gründen –
  ...
  Kundendaten, keine internen Zahlen, keine Strategiepapiere, keine
  unveröffentlichten Ankündigungen hier reinlegen – nur fertige
  Marketing-Captions und -Bilder, die ohnehin in Kürze öffentlich werden.
- Aktuell unterstützt: Bilder, Karussells (mehrere Bilder) und Videos
  ...
  automatisch auch als Story auf beiden Plattformen.
